...
import {
  TemplateValueFieldEditor,
  ITemplateValueFieldEditorProps,
} from "../../../controls/TemplateValueFieldEditor/TemplateValueFieldEditor";
...
export class PeopleLayout extends BaseLayout<IPeopleLayoutProperties> {
  ...
  public async onInit(): Promise<void> {
    ...
    if (!this.properties.peopleFields) {
      this.properties.peopleFields = [
        {
          name: strings.Layouts.People.Fields.ImageUrl,
          field: "imageUrl",
          value:
            "/_layouts/15/userphoto.aspx?size=L&username={{getUserEmail (slot item @root.slots.UserEmail)}}",
          useHandlebarsExpr: true,
          supportHtml: false,
        },
        {
          name: strings.Layouts.People.Fields.PrimaryText,
          field: "primaryText",
          value: "{{slot item @root.slots.UserDisplayName}}",
          useHandlebarsExpr: true,
          supportHtml: true,
        },
        {
          name: strings.Layouts.People.Fields.SecondaryText,
          field: "secondaryText",
          value: "JobTitle",
        ...
        {
          name: strings.Layouts.People.Fields.TertiaryText,
          field: "tertiaryText",
          value: "{{getUserEmail (slot item @root.slots.UserEmail)}}",
          useHandlebarsExpr: true,
          supportHtml: true,
        },
        {
          name: strings.Layouts.People.Fields.OptionalText,
          field: "optionalText",
          value: "WorkPhone",
          useHandlebarsExpr: false,
          supportHtml: true,
  ...
  public getPropertyPaneFieldsConfiguration(
    ...
    const availableOptions: IComboBoxOption[] = availableFields.map(
      (fieldName) => {
        return { key: fieldName, text: fieldName } as IComboBoxOption;
      }
    );
    ...
    return [
      this._propertyFieldCollectionData("layoutProperties.peopleFields", {
        ...
        panelDescription:
          strings.Layouts.People.ManagePeopleFieldsPanelDescriptionLabel,
        enableSorting: false,
      ...
      this._propertyFieldToogleWithCallout("layoutProperties.showPersonaCard", {
        ...
        calloutContent: React.createElement(
          ...
          strings.Layouts.People.ShowPersonaCardOnHoverCalloutMsg
        ),
  ...
  public async onPropertyUpdate(
    propertyPath: string,
    newValue: any,
    oldValue: any
  ) {
    if (
      propertyPath.localeCompare("layoutProperties.showPersonaCard") === 0 &&
      this.properties.showPersonaCard
    ) {
      // Load Microsoft Graph Toolkit to get the persona card
      await this.loadMsGraphToolkit();
    }
